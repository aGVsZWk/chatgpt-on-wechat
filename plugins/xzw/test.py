...
class XZW(object):
    ...
    def get_xz_name(self, seg_list):
        for seg in seg_list:
            if seg.strip() in self.constellation_dict.keys():
                return seg
        return None
...
if __name__ == '__main__':
    t = XZW()
    content = "天蝎座明日运势"
    if [key in content for key in t.constellation_dict.keys()] or "星座" in content:
        logger.info(f"[xzw] 匹配到星座【{content}】")
        seg_list = list(jieba.cut(content, use_paddle=True))  # 使用paddle模式
        is_tomorrow = t.check_is_tomorrow(seg_list)
        name = t.get_xz_name(seg_list)
        if name is not None:
            reply_text = t.get_xzw_horoscope(name, is_tomorrow)
        else:
            reply_text = "星座输入有误"
        print(reply_text)
    else:
